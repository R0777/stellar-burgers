...
const Feed: FC = () => {
    ...
    const ordersDone = orders.filter((item, index) => item.status === "done" && index < 9);
    const ordersPending = orders.filter((item, index) => item.status === "pending" && index < 9);
    return (
        <div className={clsx(s.root, "content-wrapper")}>
            ...
            <div className={s.content}>
                <div className={s.content__column}>
                    {orders.map((order, index) => (<FeedOrderCard order={order} key={index} onClick={handleClick}/>))}
                </div>
                <div className={clsx(s.content__column, "pl-15")}>
                    <div className={clsx(s.status, "mb-15")}>
                        <div className={s.status__column}>
                            <p className="text text_type_main-medium mb-6">Готовы:
                            </p>
                            <div className={s.column__feed}>
                                {ordersDone.map((item, index) => (
                                    <p
                                        key={index}
                                        className={clsx(s.text_color_bold, "text text_type_digits-default mb-1")}>
                                        {item.number}
                                    </p>
                                ))}
                            </div>
                        ...
                        <div className={s.status__column}>
                            ...
                            <div className={s.column__feed}>
                                {ordersPending.map((item, index) => (
                                    <p key={index} className={clsx("text text_type_digits-default mb-1")}>
                                        {item.number}
                                    </p>
                                ))}
                            </div>
