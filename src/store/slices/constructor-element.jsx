...
const constructorElement = createSlice({
  ...
  reducers: {
    ...
    setBulki(state, {payload}) {
        state.bredElement = state.bredElement.splice(0,1, payload)
        setTotal()
    },
      
    setMiddle(state, {payload}) {
        if (state.middleElement[0]._id === 1) {
        state.middleElement = state.middleElement.splice(0,1, payload)
        }
        else {
          state.middleElement = state.middleElement.push(payload)
        }
        setTotal()
    },
    ...
    setTotal(state) {
      const total = state.bredElement.concat(state.middleElement);
        state.total = total.forEach(el => el.price + state.total);
